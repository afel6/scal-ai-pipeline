...
// Industrial-grade Kr + Pc renderer.
// Consumes the JSON produced by KrCurveFitter.to_plot_json() / PcCurveFitter.to_plot_json()
...
import { Activity, Beaker, CheckCircle, AlertTriangle, XCircle, Info } from 'lucide-react';
  ...
  return (
    <div className="bg-[#080810]/98 backdrop-blur-2xl border border-white/10 rounded-2xl
      ...
      <div className="flex justify-between items-center mb-3 pb-2 border-b border-white/8">
        <span className="text-slate-500 uppercase tracking-widest text-[9px]">Sw</span>
      ...
      {unique.map((entry, i) => {
        const isLab = entry.name?.includes('Lab');
        return (
          <div key={i} className="flex items-center justify-between gap-6 mb-2 last:mb-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-400 truncate max-w-[100px]">{entry.name}</span>
            </div>
...
const ValidationBadge = ({ validation }) => {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[10px] font-black
                    uppercase tracking-widest transition-all ${bg} ${color}`}
      >
...
// ── PARAMETER PANEL ────────────────────────────────────────────────────────────
const ParamPanel = ({ fit_params, endpoints }) => {
...
export default function KrCurvePlot({ plotData }) {
  ...
  if (!data?.curves) {
    return (
      <div className="p-10 bg-red-950/10 border border-red-900/30 rounded-3xl text-red-400 text-xs font-mono flex flex-col items-center gap-4">
  ...
  const tickFmt = v => v >= 1000 ? v.toExponential(1) : Number(v.toFixed(3)).toString();
  ...
  return (
    <div className="my-10 rounded-[2.5rem] border border-white/5 bg-[#04040a] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.95)] overflow-hidden">
      <div className="px-8 pt-8 pb-6 border-b border-white/5 bg-gradient-to-b from-white/[0.025] to-transparent">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-[9px] font-black tracking-[0.5em] text-sky-500/70 uppercase">PRC SCAL — Kr Engine v14</span>
            </div>
            <h2 className="text-xl font-black text-white tracking-tight">{data.title}</h2>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {validation.wettability && (
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-widest ${wettConfig.bg} ${wettConfig.border}`} style={{ color: wettConfig.color }}>
        ...
        {crossover.Sw != null && (
          <div className="mt-4 inline-flex items-center gap-3 px-4 py-2 bg-white/[0.03] border border-white/8 rounded-xl text-[11px] font-mono">
      ...
      <div className="px-6 py-8 relative">
        ...
        <ResponsiveContainer width="100%" height={420}>
          <ComposedChart data={lineChartData} margin={{ top: 10, right: 60, bottom: 48, left: 10 }}>
            ...
            <XAxis dataKey="x" type="number" stroke="#ffffff10" tick={{ fontSize: 10, fill: '#475569', fontFamily: 'monospace' }} tickFormatter={tickFmt} label={{ value: data.xAxis?.label || 'Sw', position: 'insideBottom', offset: -32, fill: '#64748b', fontSize: 11, fontWeight: 700, fontFamily: 'monospace', letterSpacing: '0.1em' }} />
            <YAxis yAxisId="left" stroke="#38bdf830" tick={{ fontSize: 10, fill: '#38bdf8', fontFamily: 'monospace' }} tickFormatter={tickFmt} label={{ value: data.yAxis?.label || 'Krw', angle: -90, position: 'insideLeft', offset: 14, fill: '#38bdf8', fontSize: 10, fontWeight: 700, fontFamily: 'monospace' }} />
            {data.dualAxis && <YAxis yAxisId="right" orientation="right" stroke="#fb923c30" tick={{ fontSize: 10, fill: '#fb923c', fontFamily: 'monospace' }} tickFormatter={tickFmt} label={{ value: data.yAxis2?.label || 'Kro', angle: 90, position: 'insideRight', offset: 14, fill: '#fb923c', fontSize: 10, fontWeight: 700, fontFamily: 'monospace' }} />}
            <Tooltip content={<KrTooltip />} />
            {crossover.Sw != null && <ReferenceLine x={crossover.Sw} yAxisId="left" stroke="#facc15" strokeWidth={1.5} strokeDasharray="5 4" />}
            {data.curves.map((curve, idx) => {
              if (curve.showLine === false) return null;
              const axisId = data.dualAxis && curve.yId === 'right' ? 'right' : 'left';
              return <Line key={`line-${idx}`} yAxisId={axisId} dataKey={`line_${idx}`} name={curve.name} stroke={curve.color} strokeWidth={2.5} dot={false} type="monotone" connectNulls />;
            })}
            {data.curves.map((curve, idx) => {
              if (curve.showPoints === false) return null;
              const axisId = data.dualAxis && curve.yId === 'right' ? 'right' : 'left';
              return <Scatter key={`scatter-${idx}`} yAxisId={axisId} name={curve.name} data={curve.data} dataKey="y" fill={curve.color} stroke="#04040a" strokeWidth={1.5} r={5} />;
            })}
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="px-8 py-5 border-t border-white/5 bg-black/30 flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
          {data.curves.map((curve, idx) => (
            <div key={idx} className="flex items-center gap-2.5">
              {curve.showLine !== false ? <svg width="20" height="4"><line x1="0" y1="2" x2="20" y2="2" stroke={curve.color} strokeWidth="2.5" strokeLinecap="round" /></svg> : <div className="w-3 h-3 rounded-full border-2 border-[#04040a]" style={{ backgroundColor: curve.color }} />}
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{curve.name}</span>
            </div>
          ))}
        </div>
        <div className="flex items-center gap-2 text-slate-700 text-[9px] font-mono tracking-widest uppercase">
          ...
          <span>PRC-KR-ENGINE-V14</span>
        </div>
      </div>
      <div className="px-8 pb-8">
        <ParamPanel fit_params={fit_params} endpoints={endpoints} />
      </div>
    </div>
  );
}
