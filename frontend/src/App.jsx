...
import { Send, Paperclip, Bot, User, Download, FileText, Database, Circle, PlusCircle, Trash2, MessageSquare, ChevronLeft, ChevronRight, Wifi, WifiOff, Loader } from 'lucide-react';
...
function App() {
  ...
  const statusConfig = {
    ...
    online:  { icon: <Circle className="w-2 h-2 fill-emerald-500" />, text: 'ONLINE',        color: 'text-emerald-500' },
    ...
    <div className="min-h-screen bg-[#09090b] text-slate-100 flex font-sans overflow-hidden h-screen">
      ...
      <aside className={`flex flex-col bg-[#050505] border-r border-slate-800/60 transition-all duration-300 ${sidebarOpen ? 'w-72' : 'w-0 overflow-hidden'}`}>
        <div className="p-4 border-b border-slate-800/60 flex items-center justify-between shrink-0">
          ...
          <button onClick={handleNewChat} className="flex items-center gap-1.5 text-xs text-emerald-400 hover:text-emerald-300 border border-emerald-800/50 hover:border-emerald-600 px-2 py-1.5 rounded-lg transition-all">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          ...
          {sessions.map(s => (
            <div key={s.id} onClick={() => handleLoadSession(s.id)}
              className={`group flex items-start gap-3 px-3 py-3 rounded-xl cursor-pointer transition-all border ${s.id === sessionId ? 'bg-emerald-950/40 border-emerald-800/50 text-emerald-100' : 'border-transparent hover:bg-slate-900 hover:border-slate-800 text-slate-400 hover:text-slate-200'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-4 max-w-3xl ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border ${msg.role === 'user' ? 'bg-slate-800 border-slate-700' : 'bg-emerald-950 border-emerald-800/50'}`}>
                {msg.role === 'user' ? <User className="w-3.5 h-3.5 text-slate-300" /> : <Bot className="w-3.5 h-3.5 text-emerald-400" />}
              </div>
              <div className={`p-4 rounded-2xl text-[15px] leading-relaxed shadow-lg max-w-[85%] ${msg.role === 'user' ? 'bg-slate-800 text-slate-200 rounded-tr-none border border-slate-700/50' : 'bg-[#111116] text-emerald-50/90 rounded-tl-none border border-emerald-900/30'}`}>
                {msg.fileName && (
        ...
        <footer className="p-4 bg-[#050505] border-t border-slate-800/60 shrink-0">
          <div className="max-w-3xl mx-auto flex items-center gap-3 bg-[#111116] border border-slate-800 rounded-full p-2 pl-4 focus-within:border-emerald-500/40 transition-all">
            ...
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
              ...
              className="flex-1 bg-transparent border-none outline-none text-emerald-50 placeholder-slate-600 text-[15px] font-serif disabled:opacity-50" />
