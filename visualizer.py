...
def generate_plots(json_data: List[Dict[str, Any]], output_dir: str = None, streamlit_code: str = None) -> str:
    """
    Generates standard laboratory curves using matplotlib and saves them in the output directory.
    ...
    if porosity_plot_data:
        ...
        plt.plot(p_coords, phi_coords, marker='o', linestyle='-', color='#0F4C81', linewidth=2, markersize=6, label='Porosity')
        plt.title('Porosity vs. Overburden Pressure', fontsize=12, fontweight='bold', pad=15)
        plt.xlabel('Overburden Pressure (psi)', fontsize=10, fontweight='bold')
        plt.ylabel('Porosity (%)', fontsize=10, fontweight='bold')
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.legend(loc='best')
        plt.tight_layout()
        
        porosity_path = os.path.join(output_dir, "porosity_vs_pressure.png")
        plt.savefig(porosity_path, dpi=300)
        plt.close()
    ...
    if perm_plot_data:
        ...
        plt.plot(p_coords, k_coords, marker='s', linestyle='-', color='#10B981', linewidth=2, markersize=6, label='Air Permeability')
        plt.title('Air Permeability vs. Overburden Pressure', fontsize=12, fontweight='bold', pad=15)
        plt.xlabel('Overburden Pressure (psi)', fontsize=10, fontweight='bold')
        plt.ylabel('Air Permeability (mD)', fontsize=10, fontweight='bold')
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.legend(loc='best')
        plt.tight_layout()
