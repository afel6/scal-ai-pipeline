...
# Changes: DB connection pooling ├é┬╖ safe PG placeholder translation ├é┬╖ thread-safe

#          key rotation ├é┬╖ thread-local file URIs ├é┬╖ asyncio.Queue SSE bridge ├é┬╖

#          run_in_executor RAG ├é┬╖ transactional KB ingest ├é┬╖ admin backend auth ├é┬╖

#          env-var secrets ├é┬╖ slowapi rate limiting ├é┬╖ dead code purged
...
if DATABASE_URL:
    ...
    try:
        ...
        _logger.info("[DB] PostgreSQL pool ready (2├óΓé¼ΓÇ£15 conns)")
...
SYSTEM_PROMPT = """SYSTEM PROMPT: Senior Petrophysicist & Universal Core Data Analyst
        ...
        try:
            ...
            if name == "fit_petrophysical_curve" and args.get("model") == "micp":
                ...
                if len(pc_raw) > 1 and len(shg_raw) > 1:
                    ...
                    if has_imb:
                        ...
                        # Trapped Hg = drainage peak sat ├ó╦åΓÇÖ imbibition final sat
                    ...
                    plot_psd = {
                        ...
                        "xAxis":  {"label": "Pore Throat Radius r (├é┬╡m)"},
                    ...
                    parts = [
                        ...
                        f"Modal Pore Throat r = {thr_r:.3f} ├é┬╡m",
            ...
            if name == "fit_petrophysical_curve" and args.get("model") == "pc_centrifuge":
                ...
                if len(sw_raw) > 1 and len(pc_raw) > 1:
                    ...
                    summary = (f"Pc range: {float(pc_a.min()):.2f} ├óΓé¼ΓÇ£ {float(pc_a.max()):.2f} psia | "

                               f"Sw range: {float(sw_a.min()):.3f} ├óΓé¼ΓÇ£ {float(sw_a.max()):.3f}")
            ...
            if name == "fit_petrophysical_curve" and args.get("model") == "overburden":
                ...
                if len(pres_raw) > 1:
                    ...
                    summary = (f"Pressure range: {float(pres_a.min()):.0f} ├óΓé¼ΓÇ£ {float(pres_a.max()):.0f} psia")
        ...
        try:
            ...
            if name == "execute_python_simulation":
                ...
                _MODEL_NOTES = {
                    ...
                    "ri":            "Resistivity Index log-log plot rendered. State Archie n, compare to PRC library range n├ó╦å╦å[1.5,3.0], interpret wettability effect.",
            ...
            for attempt in range(len(self._keys)):
                ...
                try:
                    ...
                    if is_overload:
                        ...
                        yield {"type": "token", "text": "├ó┼í┬á├»┬╕∩┐╜ Gemini is currently under high demand (503). Please retry in a few seconds."}
    ...
    async def _producer():
        ...
        def _sync_worker():
            ...
            try:
                ...
                banner = "ΓÜá∩╕Å Hviel is undergoing rebuild. Outputs are NOT validated for engineering use. Do not rely on these results until further notice.\n\n"
...
async def serve_spa(full_path: str):
    ...
    # Unlike /api/download we allow nested paths (assets/js/├óΓé¼┬ª) so we use the full
